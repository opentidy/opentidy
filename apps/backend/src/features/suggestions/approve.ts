...
export function approveSuggestionRoute(deps: AppDeps) {
  ...
  router.post('/suggestion/:slug/approve', async (c) => {
    ...
    // Read suggestion before creating task (file gets moved/deleted)
    ...
    deps.workspace.taskManager.createTaskFromSuggestion(slug, body.instruction);
    ...
    deps.sse.emit({ type: 'task:updated', data: { taskId: slug }, timestamp: new Date().toISOString() });
