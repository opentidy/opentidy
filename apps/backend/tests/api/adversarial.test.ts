...
/** Minimal deps stub — only memory-related fields are real */
function makeDeps(overrides: Partial<AppDeps> = {}): AppDeps {
  return {
    workspace: {
      ...
      gapsManager: { listGaps: () => [], markResolved: () => {} },
    },
    launcher: {
      launchSession: async () => {},
      listActiveSessions: () => [],
...
describe('Adversarial memory API tests', () => {
  ...
  beforeEach(() => {
    ...
    app = createApp(makeDeps({ memoryManager, memoryAgents, workspaceDir: tmpDir }))
    r = req(app)
  })
  ...
  describe('frontmatter parsing edge cases', () => {
    it('file with no frontmatter is handled gracefully', async () => {
      ...
      const filePath = path.join(tmpDir, '_memory', 'no-front.md')
    ...
    it('file with empty frontmatter is handled', async () => {
      const filePath = path.join(tmpDir, '_memory', 'empty-front.md')
    ...
    it('file with malformed frontmatter does not crash', async () => {
      const filePath = path.join(tmpDir, '_memory', 'bad-front.md')
    ...
    it('binary content in markdown file does not crash', async () => {
      const filePath = path.join(tmpDir, '_memory', 'binary.md')
