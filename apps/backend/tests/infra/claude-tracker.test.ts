...
import { createClaudeTracker } from '../../src/infra/claude-tracker.js';
...
describe('claude-tracker', () => {
  ...
  it('records a process start and returns an id > 0', () => {
    ...
    const id = tracker.start('triage', 'dossier-abc', 12345);
    expect(id).toBeGreaterThan(0);
  });

  it('completes a process with exit code — status=done, exitCode set, endedAt set', () => {
    const tracker = createClaudeTracker(db);
    const id = tracker.start('triage', 'dossier-xyz');
  ...
  it('lists processes with optional type filter', () => {
    const tracker = createClaudeTracker(db);
    tracker.start('triage', 'dossier-1');
    tracker.start('checkup', 'dossier-2');
    tracker.start('triage', 'dossier-3');
  ...
  it('setOutputPath stores outputPath and getById returns it', () => {
    const tracker = createClaudeTracker(db);
    const id = tracker.start('triage', 'dossier-abc');
  ...
  it('cleanup removes old completed processes', () => {
    const tracker = createClaudeTracker(db);
    const id1 = tracker.start('triage', 'old-dossier');
    ...
    const id2 = tracker.start('triage', 'recent-dossier');
