// tests/receiver/watchers.test.ts
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import { createWatcher } from '../../src/receiver/watchers.js';
...
describe('Watcher', () => {
  ...
  it('polls and triages WhatsApp messages', async () => {
    const messages = [
      { from: '+33612345678', body: 'Salut, voici le doc', timestamp: '2026-03-14T10:00:00Z' },
    ];
    ...
    expect(triage).toHaveBeenCalledWith(
      expect.objectContaining({
        source: 'whatsapp',
        content: expect.stringContaining('+33612345678'),
        metadata: expect.objectContaining({ from: '+33612345678' }),
      }),
    );
  ...
  it('polls and triages SMS messages', async () => {
    const messages = [
      { from: '+49171000000', body: 'Votre colis est en route', timestamp: '2026-03-14T11:00:00Z' },
    ];
    const getNewMessages = vi.fn().mockResolvedValue(messages);
    ...
    expect(triage).toHaveBeenCalledWith(
      expect.objectContaining({
        source: 'sms',
        content: expect.stringContaining('+49171000000'),
      }),
    );
  });
  ...
  it('skips duplicate messages', async () => {
    ...
    const messages = [
      { from: '+33612345678', body: 'dupe', timestamp: '2026-03-14T10:00:00Z' },
    ];
    const getNewMessages = vi.fn().mockResolvedValue(messages);
