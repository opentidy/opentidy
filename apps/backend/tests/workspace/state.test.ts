...
describe('parseStateMd', () => {
  ...
  it('returns waitingType tiers when ATTENTE: TIERS is present', () => {
    const content = '# Test\n\nSTATUT : EN COURS\n\n## Objectif\nTest\n\n## En attente\nATTENTE: TIERS\nEmail envoye a sophie@comptable.fr\n\n## Journal\n- 2026-03-15 : Email envoye\n';
    fs.writeFileSync(path.join(tmpDir, 'state.md'), content);
    const result = parseStateMd(tmpDir);
    expect(result.waitingType).toBe('tiers');
  });

  it('returns waitingType lolo when ATTENTE: LOLO is present', () => {
    const content = '# Test\n\nSTATUT : EN COURS\n\n## Objectif\nTest\n\n## En attente\nATTENTE: LOLO\nBesoin info de Lolo\n\n## Journal\n- 2026-03-15 : Cree\n';
    fs.writeFileSync(path.join(tmpDir, 'state.md'), content);
    const result = parseStateMd(tmpDir);
    expect(result.waitingType).toBe('lolo');
  });

  it('defaults waitingType to lolo when ## En attente has no tag', () => {
...
describe('setWaitingType', () => {
  ...
  it('replaces existing ATTENTE tag in ## En attente section', () => {
    ...
    expect(result).not.toContain('ATTENTE: LOLO');
