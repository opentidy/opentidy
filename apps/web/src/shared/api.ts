...
// GET
export const fetchTasks = () => json<Task[]>('/tasks');
export const fetchTask = (id: string) => json<Task>(`/task/${id}`);
...
export const getArtifactUrl = (taskId: string, filename: string) =>
  `${BASE}/task/${encodeURIComponent(taskId)}/artifact/${encodeURIComponent(filename)}`;
...
// POST
export const createTask = (instruction: string) =>
  json('/task', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ instruction }) });
export const resumeSession = (id: string) =>
  json(`/task/${id}/resume`, { method: 'POST' });
export const sendInstruction = (id: string, instruction: string) =>
  json(`/task/${id}/instruction`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ instruction }) });
export const uploadFile = (id: string, file: File) => {
  const form = new FormData(); form.append('file', file);
  return fetch(`${BASE}/task/${id}/upload`, { method: 'POST', body: form });
};
...
export const setWaitingType = (id: string, type: 'user' | 'tiers') =>
  json(`/task/${id}/waiting-type`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type }) });
export const completeTask = (id: string) =>
  json(`/task/${id}/complete`, { method: 'POST' });
...
export const fetchClaudeProcesses = (type?: string, limit = 100) => {
  ...
  return json<AgentProcess[]>(`/claude-processes?${params}`);
};
