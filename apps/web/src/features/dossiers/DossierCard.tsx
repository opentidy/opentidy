...
export default function DossierCard({ dossier, session }: { dossier: Dossier; session?: Session }) {
  ...
  const displayStatus = isWaiting ? 'WAITING' : dossier.status;
  const config = dossierStatusConfig[displayStatus] ?? dossierStatusConfig['IN_PROGRESS'];
  const isFinished = dossier.status === 'COMPLETED';
