...
const SSE_REFRESH_MAP: Partial<Record<SSEEventType, (store: Store) => Promise<void>>> = {
  'session:started': async (s) => { await s.fetchSessions(); await s.fetchDossiers(); await s.fetchClaudeProcesses(); },
  'session:ended': async (s) => { await s.fetchSessions(); await s.fetchDossiers(); await s.fetchClaudeProcesses(); },
  'session:idle': (s) => s.fetchSessions(),
  'session:active': (s) => s.fetchSessions(),
  'dossier:updated': async (s) => { await s.fetchDossiers(); await s.fetchSessions(); },
  'dossier:completed': async (s) => { await s.fetchDossiers(); await s.fetchSessions(); },
  'suggestion:created': (s) => s.fetchSuggestions(),
  'amelioration:created': (s) => s.fetchAmeliorations(),
};

export function connectSSE(): () => void {
  const es = new EventSource('/api/events');
  const eventTypes = Object.keys(SSE_REFRESH_MAP) as SSEEventType[];
  let wasConnected = false;

  for (const type of eventTypes) {
    es.addEventListener(type, () => {
      const store = useStore.getState();
      SSE_REFRESH_MAP[type]?.(store);
    });
  }
  ...
  es.addEventListener('process:output', (e: MessageEvent) => {
    try {
      ...
      const current = _processOutputs.get(trackId) ?? '';
      _processOutputs.set(trackId, current + content);
