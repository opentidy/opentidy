...
describe('API client', () => {
  describe('GET endpoints', () => {
    ...
    it('fetchDossier calls GET /api/dossier/:id', async () => {
      const dossier = { id: 'sopra', title: 'Sopra' };
      mockFetch.mockReturnValue(mockJsonResponse(dossier));

      const result = await api.fetchDossier('sopra');

      expect(mockFetch).toHaveBeenCalledWith('/api/dossier/sopra', undefined);
  ...
  describe('POST endpoints', () => {
    ...
    it('resumeSession calls POST /api/dossier/:id/resume', async () => {
      ...
      await api.resumeSession('sopra');

      expect(mockFetch).toHaveBeenCalledWith('/api/dossier/sopra/resume', { method: 'POST' });
    });

    it('sendInstruction sends instruction with confirm', async () => {
      mockFetch.mockReturnValue(mockJsonResponse({ launched: true }));

      await api.sendInstruction('sopra', 'Do something', true);

      expect(mockFetch).toHaveBeenCalledWith('/api/dossier/sopra/instruction', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    it('approveSuggestion sends optional instruction', async () => {
      ...
      await api.approveSuggestion('impots-chypriotes', 'custom instruction');

      expect(mockFetch).toHaveBeenCalledWith('/api/suggestion/impots-chypriotes/approve', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    it('uploadFile sends FormData', async () => {
      ...
      await api.uploadFile('sopra', file);

      expect(mockFetch).toHaveBeenCalledWith('/api/dossier/sopra/upload', {
        method: 'POST',
