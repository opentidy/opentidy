...
- [ ] **Import user's agent config** — scan the user's existing Claude Code config (`~/.claude/`) during setup to discover MCP servers, skills, and plugins. Present a picker to cherry-pick what to make available in OpenTidy sessions. Imported items become modules with `source: 'imported'`, manifests built in memory (no files on disk). They appear in the modules page like any other module — activable/désactivable, permissions managed normally.
