...
class Alfred < Formula
  ...
  homepage "https://github.com/opentidy/opentidy"
  url "https://github.com/opentidy/opentidy/releases/download/v1.0.0/alfred-1.0.0.tar.gz"
  sha256 "abc123..."
...
- **Source**: GitHub Releases API (`GET /repos/opentidy/opentidy/releases/latest`)
- **Flow**:
...
**Migration note**: The current `com.opentidy.agent.plist` contains hardcoded Telegram tokens. This must be migrated to `~/.config/alfred/config.json` and the plist updated to reference config via environment variables loaded by Alfred at startup. The exposed token in the committed plist should be rotated.
...
2. Runs `brew tap opentidy/opentidy && brew install alfred`
