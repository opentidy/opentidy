...
OpenTidy uses Telegram for push notifications — checkpoint alerts, dossier completions, error reports. Each notification includes a link to the web dashboard.
...
The workspace directory contains all dossiers, suggestions, gaps, and audit logs. It's created automatically on first run.
...
├── <dossier-id>/
│   ├── CLAUDE.md            # Per-dossier prompt (level 2, auto-generated)
│   ├── state.md             # Dossier state, progress, next steps
...
- [Getting Started](getting-started.md) — installation and first dossier
